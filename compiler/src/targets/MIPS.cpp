...
void Writer::ADDI(const MIPS::Register &dest, const MIPS::Register &src,
                  const int val) {
  atl::string output = "ADDI " + dest.toString() + ", " + src.toString() +
                       ", " + atl::to_string(val);
  output += "\t# " + dest.toString() + " = " + src.toString() + " + " +
            atl::to_string(val);
  write(output);
}
...
void Writer::JAL(const atl::string &blockName) { write("JAL " + blockName); }

void Writer::LW(const MIPS::Register &destReg, const int addr,
                const int offset) {
  atl::string output = "LW " + destReg.toString() + ", " +
                       atl::to_string(offset) + "(" + atl::to_string(addr) +
                       ")";
  output += "\t# " + destReg.toString() + " = " + atl::to_string(offset) + "(" +
            atl::to_string(addr) + ")";
  write(output);
}
...
void Writer::SW(const MIPS::Register &regCtnt, const int addr,
                const int offset) {
  atl::string output = "SW " + regCtnt.toString() + ", " +
                       atl::to_string(offset) + "(" + atl::to_string(addr) +
                       ")";
  output += "\t# " + atl::to_string(offset) + "(" + atl::to_string(addr) +
            ") = " + regCtnt.toString();
  write(output);
}
...
void Writer::SW(const MIPS::Register &regCtnt, const MIPS::Register &addr,
  ...
  atl::string output = "SW " + regCtnt.toString() + ", " +
                       atl::to_string(offset) + "(" + addr.toString() + ")";
  output += "\t# " + atl::to_string(offset) + "(" + addr.toString() +
            ") = " + regCtnt.toString();
  write(output);
}
