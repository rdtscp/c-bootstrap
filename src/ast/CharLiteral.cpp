...
bool CharLiteral::operator!=(const CharLiteral &rhs) const { return !(*this == rhs); }
...
bool CharLiteral::operator==(const Stmt &rhs) const {
  if (rhs.astClass() == astClass())
    return *this == *static_cast<const CharLiteral *>(&rhs);
  return false;
}

bool CharLiteral::operator!=(const Stmt &rhs) const { return !(*this == rhs); }
